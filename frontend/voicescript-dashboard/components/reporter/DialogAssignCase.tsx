...
export default function DialogAssignCase({ reporterId, location, isOpen, onClose, onSuccess }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchCase = async () => {
      try {
        const data = await apiFetch<Case[]>(`${API.case}/available/reporter?location=${location}&status=new`);
        if (!cancelled) {
          setCase(data);
          setLoading(false);
        }
  ...
  async function handleSubmit(caseId: number) {
    ...
    setApiError(null);
    try {
      const payload = {reporterId: reporterId, caseId: caseId}
      await apiPost(`${API.case}/assign/reporter`, payload);
      // setForm(defaultForm);
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-xl shadow-xl w-full max-w-fit">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b">
            <h3 className="text-lg font-semibold">Assign Case Reporter</h3>
            <button
              onClick={onClose}
